...
from rag.recommendation.v3.clarification_policy import ClarificationPolicy
from rag.recommendation.v3.semantic_contracts import CartObservation
from rag.recommendation.v3.session import apply_session_delta, empty_session_core, load_session_core, recommendation_delta
from rag.recommendation.v3.types import CardModel, CartOperation, CartTargetRef, CartTargetSource, RequirementSpecV3, V3Action
...
def test_cart_add_is_plan_then_confirm_and_uses_card_catalog_reference():
    ...
    observation = CartObservation(operation=CartOperation.ADD, target_ref=CartTargetRef(CartTargetSource.CARD, 1), quantity=2)
...
def test_cart_cancel_and_expiry_cannot_apply_a_plan():
    catalog = load_product_catalog()
    core = _core_with_card()
    observation = CartObservation(operation=CartOperation.ADD, target_ref=CartTargetRef(CartTargetSource.CARD, 1))
    plan = create_cart_plan(core=core, observation=observation, catalog=catalog, now=100.0)
...
def test_cart_target_source_must_match_the_operation():
    clarification = ClarificationPolicy().plan(
        observation=CartObservation(operation=CartOperation.ADD, target_ref=CartTargetRef(CartTargetSource.CART, 1)),
        core=empty_session_core(),
    )
    assert clarification is not None
    assert clarification.reason_code == "cart_target_schema_conflict"
