...
flowchart TD
    ...
    D -- 否 --> P0["[4a] 立即发送 progress(stage=understanding)<br/>只表示已收到请求，不含模型结论"]
    P0 --> F["[5] normalize_turn()<br/>NFKC、删除零宽/控制字符、合并空白"]
...
1. **先防错，再理解。** `sanitize_input()` 不会“猜”用户的意思；它只拦空请求、超长文本和明显提示注入。通过检查且没有附件后，服务会立刻发送 `progress(stage=understanding)`，让前端知道请求已经被接收；它没有任何推荐结论。`normalize_turn()` 再只统一字符和空白，保留原话作为后续证据来源。
...
| 4a | `chat_stream` 内第一个 `progress` 事件 | 在调用 Router、Chat 模型、embedding 或 Milvus 前发送“正在理解需求”。这是纯 UI 回执，不能展示半截 JSON、商品名或业务结论。 |
...
- 首个可显示事件、首个业务结果、总延迟、模型调用数、供应商返回 token、Milvus 故障降级和并发正确率。
...
> 延迟口径已在 2026-07-18 调整：旧 61 轮报告里的“首事件”发生在路由完成之后，因而约为
> 5--6 秒；当前链路会先发送不带业务结论的 `progress(stage=understanding)`。从本次改动开始，报告把
> **首个可显示事件延迟**与**首个业务结果延迟**分开，旧数值不能再拿来描述用户首次看到界面的等待时间。

本次改动后的两条真实冒烟请求见 [首事件冒烟结果](final_test/results/first_visible_event_smoke_20260718.md)：
首个可显示事件为 **19 ms、42 ms**（N=2，均值 **30.5 ms**）；其中外部 Chat + Milvus 的业务结果仍需
**8039 ms**，因此不能把“即时进度反馈”误写成“模型或商品推荐已经在 30 ms 内完成”。
