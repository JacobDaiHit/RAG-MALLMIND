...
class ParentChunkStore:
    ...
    def _to_dict(item: ParentChunk) -> dict:
        """父子分片存储：封装 to dict 相关逻辑，供上层流程复用。"""
        return {
            "text": item.text,
    ...
    def _cache_key(chunk_id: str) -> str:
        """父子分片存储：封装 cache key 相关逻辑，供上层流程复用。"""
    ...
    def upsert_documents(self, docs: List[dict]) -> int:
        ...
        try:
            ...
            logger.exception("Parent chunk upsert failed")
            db.rollback()
            raise
        finally:
            db.close()
    ...
    def get_documents_by_ids(self, chunk_ids: List[str]) -> List[dict]:
        """父子分片存储：获取 documents by ids，屏蔽配置、缓存或外部依赖细节。"""
        if not chunk_ids:
            return []
    ...
    def delete_by_filename(self, filename: str) -> int:
        ...
        try:
            ...
            logger.exception("Parent chunk delete failed")
            db.rollback()
            raise
        finally:
            db.close()
