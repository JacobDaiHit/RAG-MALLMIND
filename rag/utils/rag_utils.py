...
# Legacy compatibility helpers. The active recommendation retrieval path uses
# rag.utils.retrieval_postprocess for rerank and auto-merge post-processing.
...
def _get_rerank_endpoint() -> str:
    """RAG 工具函数：获取 rerank endpoint，屏蔽配置、缓存或外部依赖细节。"""
...
def _merge_to_parent_level(docs: List[dict], threshold: int = 2) -> Tuple[List[dict], int]:
    """RAG 工具函数：封装 merge to parent level 相关逻辑，供上层流程复用。"""
...
def _auto_merge_documents(docs: List[dict], top_k: int) -> Tuple[List[dict], Dict[str, Any]]:
    """RAG 工具函数：封装 auto merge documents 相关逻辑，供上层流程复用。"""
...
def _rerank_documents(query: str, docs: List[dict], top_k: int) -> Tuple[List[dict], Dict[str, Any]]:
    """RAG 工具函数：封装 rerank documents 相关逻辑，供上层流程复用。"""
    ...
    try:
        ...
        if response.status_code >= 400:
            logger.warning("Legacy rerank API returned HTTP %s", response.status_code)
        ...
        logger.warning("Legacy rerank postprocess failed: %s", e)
        meta["rerank_error"] = public_error(e, fallback="rerank_failed")
        return docs_with_rank[:top_k], sanitize_report(meta)
